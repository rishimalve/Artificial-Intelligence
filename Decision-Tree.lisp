...
(print "***final tree***")
(print *t*)
;(print "***orginial data set labels***")
;(print (mapcar #'label-of-example *restaurant-examples*))
;(print "***tree generated labels***")
;(print (mapcar #'(lambda (x) (find-label-for-example x *t*)) *restaurant-examples*))


;;; The decision tree got wrong 3 labels in the generalization step.
;;; Original labels:  (T NIL T T NIL T NIL T NIL NIL NIL T)
;;; Generalization:   (T NIL T NIL NIL T NIL T T T NIL T)

;;; Report:
;;; The code implements the Decision Tree algorithm, which takes random samples of data and features and generalises it to a tree with a least possible travel which divides the dataset.
;;; Execution of the code starts with the call to the function build-decision-tree which calls the choose-feature function which returns the next feature to be selected for building the tree.
;;; choose-feature function uses the remainder and information functions to make decision on the next feature to be choses. This is the basic flow of the code.
;;; 
;;; The build-decision-tree function is on the same lines with the algorithm given in the class notes. Default label of the tree is choses as the most common label in the example dataset.
;;; The function creates a local variable tree as an empty list at each recursive call which will be returned after each call ends. Then I've used nested conditional statements using cond block
;;; which checks the below base condition of the recursion.
;;;   1. example data set is empty
;;;   2. all the examples in the dataset have the same label.
;;;   3. we are no longer left with any feature definitions.
;;; If any of the above conditions are not satisfied the function calls the build-decision-tree function recursively with reduced example and feature-def set. Otherwise base condition returns the 
;;; correct label of the for the branch as per the algorithm.
;;; 
;;; Choose-feature function takes returns the feature which best divides the examples. It used remainder function which provides the remainder of each feature-def.
;;; I've created a list called remainder-list which stores the feature-def and its remainder. A loop calls the remainder function for each of the features and the returned remainder is stored
;;; in the remainder-list list along with its feature name. Then a loop traverses the entire remainder-list and chooses the feature with the least remainder which is returned to the build-decision-tree
;;; function.
;;; 
;;; Remainder function returns the sum which is the product of the probability of the feature taking each value and the information content of each label of the feature. To calculate the
;;; information of the labels, remainder function calls the information function internally. This function has two local variables, final-rem and feature-probabilities. List feature-probabilities
;;; which stores the probability of the value of each label of the feature to the total label count of the feature. Final-rem is counted which is product of the information value returned 
;;; for each label of the feature. This function internally uses helper function such as num-examples-with-feature-val, num-examples-with-label and examples-with-feature-val which are useful
;;; in calculating the probabilities.
;;; 
;;; Information function takes the probabilities of each value of the feature and returns the sum i.e. the information stored in the feature to the remainder function. It loops over the 
;;; list of the probabilities and calculates the sum of the product of each probability and the log of the probability to the base 2.
;;; 
;;; I have also checked working of this code on the department-room dataset and it gives the tree same as the tree which has been provided in the class notes.