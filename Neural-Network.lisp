...
(defun net-error (output correct-output)
  ...
  (scalar-multiply 0.5 (multiply (transpose (subtract correct-output output)) (subtract correct-output output))))
...
the datum as input."
  ...
  (map-m #'sigmoid (multiply w (map-m #'sigmoid (multiply v (car datum))))))
...
returning a list consisting of new, modified V and W matrices."
  ...
  (let ((odelta nil) (hdelta nil) (h (map-m #'sigmoid (multiply v (car datum)))))
    (setf odelta (e-multiply (subtract (nth 1 datum) (forward-propagate datum v w)) (e-multiply (forward-propagate datum v w) (subtract-from-scalar 1 (forward-propagate datum v w)))))
    (setf hdelta (e-multiply h (e-multiply (subtract-from-scalar 1 h) (multiply (transpose w) odelta))))
    (setf w (add w (scalar-multiply alpha (multiply odelta (transpose h)))))
    (setf v (add v (scalar-multiply alpha (multiply hdelta (transpose (car datum))))))
    (list v w)))
...
and the final W matrix of the learned network."
    ...
    (setf initial-bounds 0.01)
    (let ((v nil) (w nil) (p nil) (o nil) (err nil) (v-w nil))
        (setf v (make-random-matrix num-hidden-units (list-length (car (car data))) initial-bounds))
        (setf w (make-random-matrix (list-length (cdr (car data))) num-hidden-units initial-bounds))
        (dotimes (i max-iterations)
          (setf p o)
          (let ((sdata nil))
              (setf sdata (shuffle data))
              (dolist (j sdata)
                (setf v-w (back-propagate j alpha v w))
                (setf v (nth 0 v-w))
                (setf w (nth 1 v-w)))
              (if (equalp (mod i modulo) 0)
                (progn 
                  (format t "~%Iteration: ~a" i)
                  (setf err nil)
                  (dolist (j sdata)
                    (setf o (forward-propagate j v w))
                    (setf err (nconc err (list (optionally-print (car (car (net-error o (nth 1 j)))) print-all-errors))))
                    (setf err (nconc err (list (car (car (net-error o (nth 1 j))))))))
                  (format t "~%Worst Error: ~a" (apply 'min err))
                  (format t "~%Mean Error: ~a" (average err))
                  (if (< (apply 'min err) *a-good-minimum-error*) (return ))))))
     (list v w)))
...
and use a modulo of MAX-ITERATIONS."
  ...
  (let ((v nil) (w nil) (n-data nil) (lst nil) (o nil))
    (print "-------------------------Training Step-------------------------")
    (setf lst (net-build (convert-data (butlast data (floor (/ (list-length data) 2)))) num-hidden-units alpha initial-bounds max-iterations 1000 t))
    (setf v (nth 0 lst))
    (setf w (nth 1 lst))
    (setf n-data (convert-data (last data (floor (/ (list-length data) 2)))))
    (setf lst nil)
    (print "----------------------Generalization Step----------------------")
    (dolist (j n-data)
      (setf o (forward-propagate j v w))
      (setf lst (nconc lst (list (car (car (net-error o (nth 1 j))))))))
    (format t "~%Mean Error: ~a" (average lst))))
...
and use a modulo of MAX-ITERATIONS."
  (let ((v nil) (w nil) (lst nil) (data1 data)(n-data nil) (o nil))
    (dotimes (i k)
      (print "-------------------------Training Step-------------------------")
      (format t "~%K: ~a" (+ 1 i))
      (setf lst (net-build (convert-data (butlast data1 (floor (* (/ (- k 1) k) (list-length data1))))) num-hidden-units alpha initial-bounds max-iterations 1000 t))
      (setf v (nth 0 lst))
      (setf w (nth 1 lst))
      (setf n-data (convert-data (last data1 (floor (* (/ 1 k) (list-length data1))))))
      (setf lst nil)
      (print "----------------------Generalization Step----------------------")
      (dolist (j n-data)
        (setf o (forward-propagate j v w))
...
;(simple-generalization *voting-records* 3 1.0 5 5000)
;(k-fold-validation *voting-records* 5 3 1.0 5 5000)

;(k-fold-validation *nand* 5 3 1.0 5 20000)

;(net-build (convert-data *wine*) 10 1.0 2 5000 250 t)


;(net-build (convert-data (scale-data *mpg*)) 10 1.0 2 5000 250 t)
